...
- 
- `for char in text`: char_hex, char_width = char_to_hex(char, matrix_height, font=font, font_offset=font_offset, font_size=font_size)
...
- return result.lower()
- 
- `def send_text(text, rainbow_mode=0, animation=0, save_slot=1, speed=80, color="ffffff", font="default", font_offset_x=0, font_offset_y=0, font_size=0, matrix_height=16)`: - """Send a text to the device with configurable parameters.
...
- `if font_size == 0`: font_size = matrix_height
- 
- # Disable unsupported animations (bootloop)
- `if animation == 3 or animation == 4`: raise ValueError("Invalid animation for text display")
- 
